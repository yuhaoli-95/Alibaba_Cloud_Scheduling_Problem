...
int main()
{
    ...
    std::unordered_set<int> processor_ids;
    std::vector<std::vector<int>> pt_matrix;
    std::ifstream ifs_mat("../tcdata/process_time_matrix.csv");
    ...
    std::ifstream ifs_order("../tcdata/work_order.csv");
    ...
    std::ifstream ifs_result{"../result.csv"};
    ...
    int a{3}, b{2}, c{3000};
    double score = c * avg_efficiency / (a * avg_timeout_response + b * delta_loads);

    std::cout << "R: " << avg_efficiency << "; M: " << avg_timeout_response << "; deltaL: " << delta_loads << std::endl;
